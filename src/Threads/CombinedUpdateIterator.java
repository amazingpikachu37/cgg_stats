...
public class CombinedUpdateIterator extends UpdateIteratorAbstraction {
    ...
    private final TreeMap<UpdateValue, UpdateIterator> next_counts = new TreeMap<>((UpdateValue k1,UpdateValue k2) -> Long.signum(k1.getUUID().getTime()-k2.getUUID().getTime()));
    public CombinedUpdateIterator(List<Statable> threads, UUID uuid1, UUID uuid2) {
        this.threads = threads;
        this.uuid1 = uuid1;
        this.uuid2 = uuid2;
    }

    public CombinedUpdateIterator init() {
        for(Statable thread:threads) {
            UpdateIterator thread_counts = new UpdateIterator(thread, uuid1, uuid2).init();
    ...
    public UpdateValue next() {
        if(this.hasNext()) {
            Map.Entry<UpdateValue, UpdateIterator> next_count = next_counts.pollFirstEntry();
