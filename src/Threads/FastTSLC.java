...
public class FastTSLC extends NoMistakesThread {
    public FastTSLC() {
        super("fast_tslc", List.of("fast_tslc"), "America/Montreal");
    }
    ...
    public List<Integer> getGetValues() {
        ...
        for(int i=0;i<=this.getMaxCountLocation().getChatFile();i++) {
            ...
            for (int j=0;j<nums.size();j++) {
                try {
                    if (Integer.parseInt(nums.get(j).getRawCount()) % 50 == 0) {
                        gets.add(nums.get(j).getValidCountNumber());
                        j+=49; //Minimum amount of counts between gets
                    }
    ...
    public String getNextGet() {
        return "The next get is `"+ getCount((this.getLast_number()/50+1)*50)+"`";
    }
    public String[] link() {
        return new String[]{"https://counting.gg/thread/fast_tslc"};
    }
